...
describe('HashService', () => {
  ...
  describe('hash', () => {
    test('should call argon2.hash with the provided data', async () => {
      await hashService.hash(dataToHash)
      expect(argon2.hash).toHaveBeenCalledWith(dataToHash)
    })

    test('should return the hashed data', async () => {
  ...
  describe('compare', () => {
    test('should call argon2.verify with the provided data and hash', async () => {
    ...
    test('should return false if data does not match the hash', async () => {
